...
# Generate an optimaized Verilog module that implements a Moore FSM using lib based on input truth table
...
def generate_verilog_module(output_file, output_labels, input_labels, gate_wire_dict, wire_output_dict):    
    ...
    output_file.write("\twire val;\n\n")
    output_file.write("\tassign val = 1'b1;\n\n")

    for output in output_labels:
        output_file.write("\tdff$ dff_" + output + " (clk, " + wire_output_dict[output] + ", " + output + ", val, val);\n")
